...
function ErrorState({ message }: { message: string }) {
  return (
    <div className="p-6">
      <div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-4 text-sm text-red-800">{message}</div>
    </div>
  );
}
    ...
    <section className="flex min-h-0 min-w-0 flex-1 flex-col bg-white">
      ...
      {!isLoading && !errorMessage ? (
        <div className="min-h-0 flex-1 overflow-auto bg-white">
          <div className="min-h-full min-w-max font-mono text-[13px] leading-6 text-slate-800">
            {lines.map((line, index) => (
              <div key={`${path}-${index + 1}`} className="grid grid-cols-[4rem_minmax(0,1fr)]">
                <div className="select-none border-r border-line bg-slate-50 px-4 py-0.5 text-right text-xs text-slate-400">
                  {index + 1}
                </div>
